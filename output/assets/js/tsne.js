  ...
  this.size = {
    ...
    texture:
      window.innerWidth < this.mobileBreakpoint
        ? smallTexSize
        : webgl.limits.textureSize,
    lodTexture:
      window.innerWidth < this.mobileBreakpoint ? smallTexSize : bigTexSize, // one detail texture buffer
    points: {
...
Layout.prototype.showHideIcons = function () {
  var display =
    window.innerWidth < config.mobileBreakpoint ? "none" : "inline-block";
      ...
      get(
        ...
        function (pos) {
          ...
          if (lines.mesh) {
            ...
            lines.mesh.geometry.attributes.targetPosition.array =
              lineAttributes.positions.array.slice(0);
            lines.mesh.geometry.attributes.targetPosition.needsUpdate = true;
            lines.mesh.geometry.attributes.targetMidpoint.array =
              lineAttributes.midpoints.array;
...
Layout.prototype.setPointScalar = function () {
  ...
  if (l == "tsne" || l == "umap" || l == "pose")
    size = config.size.points.scatter;
...
Layout.prototype.onTransitionComplete = function () {
  ...
  if (lines.mesh) {
    lines.mesh.geometry.attributes.position.array =
      lines.mesh.geometry.attributes.targetPosition.array.slice(0);
    lines.mesh.geometry.attributes.midpoint.array =
      lines.mesh.geometry.attributes.targetMidpoint.array.slice(0);
...
Lasso.prototype.handleMouseUp = function (e) {
  ...
  if (
    (e.clientX || e.pageX) == this.mousedownCoords.x &&
...
Picker.prototype.onMouseUp = function (e) {
  ...
  if (
    ...
    // whether the click hit the gl canvas
    return;
  }
...
Lines.prototype.plot = function () {
  ...
  get(
    getPath("data/image-vectors/openpose/vectors.json"),
    function (json) {
      this.json = json;
      // draw lines
      var material = new THREE.RawShaderMaterial({
        vertexShader: document.querySelector("#line-vertex-shader").textContent,
        fragmentShader: document.querySelector("#line-fragment-shader")
          .textContent,
        uniforms: {
      ...
      this.elems.container.style.display = "inline-block";
      this.elems.select.addEventListener(
        "change",
        this.toggleLineDisplay.bind(this)
      );
    }.bind(this)
  );
};
...
Lines.prototype.getAttributes = function (vertexType) {
  ...
  this.json.forEach(function (body, bodyIdx) {
    ...
    body.forEach(function (vert, vertIdx) {
      ...
      var y = 1 - vert[1];
      var keys =
        vertexType == "position" ? { x: "x", y: "y" } : { x: "tx", y: "ty" };
      vertices.push({
    ...
    var n = vertices.filter(function (v) {
...
Modal.prototype.showCells = function (cellIndices, cellIdx) {
  ...
  function showModal(json) {
    ...
    document
      .querySelector("#svg-image-filtered")
      .setAttribute("href", json.image.src);
    document
      .querySelector("#svg-image-unfiltered")
      .setAttribute("href", json.image.src);
    // add the click listener to the pose icon if relevant
    if (hasPose) {
      document
        .querySelector("#pose-highlight-icon")
        .addEventListener("click", self.togglePoseHighlight.bind(self));
...
function getPath(path) {
  ...
  base += path
    .replace("\\", "/")
    .replace(config.data.output_directory + "/", "");
  return base;
}
